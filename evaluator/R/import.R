...
split_sheet <- function(dat, table_type = "capabilities") {
  # -------------------------------------------------------------------------
  # NOTA DE LOCALIZACIÓN (app TFM): esta función fue parcheada para soportar
  # plantillas con 0 filas de escenarios. Antes, la rama "threats" ejecutaba
  # `dat[(split_point + 2):nrow(dat), ]` y, cuando no existían filas de datos,
  # el índice descendente (p. ej. 9:8) devolvía la fila del encabezado como si
  # fuera un escenario, generando filas basura en qualitative_scenarios.csv.
  # Ahora se devuelve un marco de 0 filas con los nombres del encabezado.
  # -------------------------------------------------------------------------

  # Eliminar las filas completamente vacías del rango leído (openxlsx deja
  # celdas en blanco al limpiar la plantilla; readxl las devuelve como NA).
  dat <- dat[rowSums(!is.na(dat)) != 0, , drop = FALSE]

  # Localizar la fila donde comienza la tabla de amenazas ("Threats")
  split_point <- which(dat$Name == "Threats")

  if (table_type == "capabilities") {
    # --- Sección de Capabilities (controles) --------------------------------
    # Si no hay filas de controles (split_point <= 2), devolver marco vacío.
    if (length(split_point) == 0 || split_point <= 2) {
      return(dat[0, , drop = FALSE])
    }
    # Los controles ocupan las filas anteriores a "Threats"
    capabilities_data <- dat[1:(split_point - 1), , drop = FALSE]
    ...
    # --- Sección de Escenarios (amenazas) -----------------------------------
    if (length(split_point) == 0) {
      stop("No se encontró la fila 'Threats' en la hoja.", call. = FALSE)
    }
    if (nrow(dat) >= split_point + 2) {
      # Hay al menos una fila de datos: tomarlas todas desde la fila posterior
      # al encabezado (split_point + 1 es el encabezado de columnas).
      threats_data <- dat[(split_point + 2):nrow(dat), , drop = FALSE]
    } else {
      # Plantilla VACÍA: solo existe el encabezado de columnas. Devolver un
      # marco de 0 filas con los nombres del encabezado para que el pipeline
      # continúe sin errores y sin inventar escenarios.
      threats_data <- dat[0, , drop = FALSE]
    }
    names(threats_data) <-
