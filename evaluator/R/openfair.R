...
sample_lm <- function(n, .func = NULL, params = NULL) {
  ...
  # Límite máximo de pérdida (cap): si `params` trae `max` y la función de
  # muestreo NO lo usa como parámetro nativo (pert/unif sí lo usan como cota
  # superior), se retira antes de llamar a la RNG y se aplica después con
  # pmin(). Así `dist:lnorm|params:meanlog=...,sdlog=...,max=50000` limita la
  # pérdida para CUALQUIER distribución (weibull, exp, gamma, ...).
  # NOTA: `params` puede llegar como lista o como vector nombrado (unlist en
  # openfair_tef_tc_diff_lm), por eso se usa `[[` en lugar de `$`.
  cap <- NULL
  if (!is.null(params) && "max" %in% names(params) &&
      !("max" %in% names(formals(.func)))) {
    cap <- params[["max"]]
    params <- params[names(params) != "max"]
  }
  samples <- if (!is.na(n) && n != 0) rlang::exec(.func, n = n, !!!params) else 0
  if (!is.null(cap) && !is.na(cap) &&
      length(samples) > 0 && !all(is.na(samples))) {
    samples <- pmin(samples, cap)
  }
...
select_loss_opportunities <- function(tc, diff, n = NULL, ...) {
  # With no controls (inherent risk), difficulty is zero: every threat event
  # with positive capability is a loss opportunity.
  if (length(diff) == 0) diff <- rep(0, length(tc))
  samples <- tc > diff
  samples <- if (!is.null(n) && n <= length(samples)) samples[1:n] else samples
